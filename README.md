...
Pierwszych osiem filtrów jest spiętych automatycznie przez `pipeline_v3.validate_all()`.
`symbolic_logic_filter` i `linalg` mają osobne tryby wejścia (`formula=True`,
`matrix=True`), bo operują na innej gramatyce niż zwykłe wyrażenia algebraiczne.
